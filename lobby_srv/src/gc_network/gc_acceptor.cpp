...
namespace gc {
	...
	{
		REG_MSG(protocol::connect_lobby_req);
		REG_MSG(protocol::tutorial_progress_update_req);
		REG_MSG(protocol::first_vehicle_select_req);
		REG_MSG(protocol::asset_info_req);

		REG_MSG(protocol::player_profile_info_req);
		REG_MSG(protocol::update_player_profile_info_req);
		REG_MSG(protocol::match_info_req);
		REG_MSG(protocol::match_start_req);
		REG_MSG(protocol::match_stop_req);
		REG_MSG(protocol::match_item_buy_req);

		REG_MSG(protocol::vehicle_list_req);
		REG_MSG(protocol::vehicle_select_req);
		REG_MSG(protocol::vehicle_upgrade_req);
		REG_MSG(protocol::vehicle_parts_tuning_req);

		REG_MSG(protocol::ability_list_req);
		REG_MSG(protocol::ability_slot_open_req);
		REG_MSG(protocol::ability_equip_req);
		REG_MSG(protocol::ability_acquire_req);

		REG_MSG(protocol::grandprix_info_req);
		REG_MSG(protocol::grandprix_rank_info_req);

		REG_MSG(dev::put_vehicle_card_req);
		REG_MSG(dev::put_vehicle_stuff_req);

		REG_MSG(protocol::attendance_list_req);
		REG_MSG(protocol::attendance_receive_reward_req);

		REG_MSG(protocol::daily_mission_list_req);
		REG_MSG(protocol::daily_mission_receive_reward_req);

		REG_MSG(protocol::achievement_list_req);
		REG_MSG(protocol::achievement_receive_reward_req);
	}
